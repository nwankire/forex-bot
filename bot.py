...
PAIRS = ["EUR/USD", "GBP/USD", "USD/JPY", "AUD/USD", "USD/CAD", "EUR/GBP", "EUR/JPY", "GBP/JPY", "AUD/JPY", "NZD/USD", "USD/CHF", "EUR/AUD", "GBP/AUD", "EUR/CAD"]
TIMEFRAME = "5min"
EXPIRY = "5 Minutes"
SCAN_INTERVAL = 4
BOT_ACTIVE = True
...
def get_data(pair, interval="5min"):
    try:
        ...
        params = {"symbol": symbol, "interval": interval, "outputsize": 30, "apikey": API_KEY}
...
def check_signal(pair):
    ...
    if not data or len(data["close"]) < 21: return None
    closes = data["close"]
    ema9, ema21, rsi14 = ema(closes, 9), ema(closes, 21), rsi(closes, 14)
    if not ema9 or not ema21 or not rsi14: return None
    prev_ema9, prev_ema21 = ema(closes[:-1], 9), ema(closes[:-1], 21)
    if prev_ema9 <= prev_ema21 and ema9 > ema21 and 40 < rsi14 < 60 and closes[-1] > closes[-2]:
        return {"pair": pair, "signal": "CALL", "entry": closes[-1], "rsi": round(rsi14, 1)}
    elif prev_ema9 >= prev_ema21 and ema9 < ema21 and 40 < rsi14 < 60 and closes[-1] < closes[-2]:
        return {"pair": pair, "signal": "PUT", "entry": closes[-1], "rsi": round(rsi14, 1)}
    return None

def is_session_active():
    return 8 <= datetime.now(TZ).hour < 17

async def scan_and_send(context: ContextTypes.DEFAULT_TYPE):
    if not BOT_ACTIVE or not is_session_active(): return
    for pair in PAIRS:
...
async def tf_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args: return await update.message.reply_text("Usage: /tf M1|M5|M15")
    tf = context.args[0].upper()
    if tf == "M1": TIMEFRAME, EXPIRY, SCAN_INTERVAL = "1min", "1 Minute", 1
    elif tf == "M5": TIMEFRAME, EXPIRY, SCAN_INTERVAL = "5min", "5 Minutes", 4
    elif tf == "M15": TIMEFRAME, EXPIRY, SCAN_INTERVAL = "15min", "15 Minutes", 15
    else: return await update.message.reply_text("Invalid. Use: M1, M5, or M15")
...
def main():
    ...
    async def post_init(app): app.web_app.router.add_get("/", health)
    ...
    application.run_webhook(listen="0.0.0.0", port=PORT, webhook_url=f"{WEBHOOK_URL}/", url_path="")
